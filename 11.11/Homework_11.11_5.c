...
//答案：
/*
方法一：（负数无法计算！！！！  如果想让输入负数也可以，必须将int换成unsigned int，认为二进制中的32位1都为有效数
思路：
循环进行以下操作，直到n被缩减为0：
   ...
   3. 如果n不等于0时，继续1
*/
//int count_one_bit(int n)  //如果直接这样传入-1，-1不为假，进入循环，-1%2为0，不进入if，n=n/2为0，直接跳出循环
//{
//	int count = 0;
//	while (n)
//	{
...
*/
//int count_one_bit(int n)  //答案印错，这里应该直接是int
//{
//	int count = 0;
//	int i = 0;
//	for (i = 0; i < 32; i++)
